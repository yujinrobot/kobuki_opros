...
public:
	void save(opros::archive::oarchive &ar, unsigned int version) {
		ar << OPROS_SERIALIZATION_NVP(m_image);
	}

	void load(opros::archive::iarchive &ar, unsigned int version) {
		ar >> OPROS_SERIALIZATION_NVP(m_image);
	}
...
inline void save_this(opros::archive::oarchive &ar, RawImageData &g)
{
	...
	ar.save((char *)g.getData(), (const long)size);
}
 ...
 *
  ...
  RawImageData *pimage = myImage.getImage();

  pimage->resize( imageWidth, imageHeight, bytePerPixel);   // ex: bytePerPixel => 3 for RGB 24bit
  int total_byte_size = imageWidth*imageHeight*bytePerPixel;

  unsinged char *imageBuffer  = pimage->getData();

  memcpy(imageBuffer, myCaptureImageBuffer, total_byte_size);

  .... use image .....................

  void myView(RawImage &pdib)
  {
  	RawImageData *prdib = pdib.getImage();

	unsigned char *imageDataBuffer = prdib->getData();
